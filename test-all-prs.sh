...
for branch_info in "${PR_BRANCHES[@]}"; do
    ...
    git checkout "origin/$branch_name" -b "test-$branch_name" 2>/dev/null || {
        git checkout "test-$branch_name" 2>/dev/null || true
        git reset --hard "origin/$branch_name"
    }
    
    # Try to build
    echo "Attempting build..."
    BUILD_OUTPUT=$(timeout 300 ./gradlew assembleDebug --no-daemon 2>&1 || echo "BUILD_FAILED")
    ...
    if echo "$BUILD_OUTPUT" | grep -q "BUILD SUCCESSFUL"; then
        ...
        ERROR_SUMMARY=$(echo "$BUILD_OUTPUT" | grep -E "e: file://" | head -3 | tr '\n' '; ')
    fi
    ...
    if [ "$BUILD_STATUS" = "❌ FAILED" ]; then
        echo "- **Errors**: $ERROR_SUMMARY" >> "$RESULTS_FILE"
    fi
    echo "" >> "$RESULTS_FILE"
